...
function calcularCobrosCliente($id)
{
    $GLOBALS['a']->select(
        'cobros',
        'costo_cliente + estadias_cliente + lavado_cliente + burreo_cliente + demoras_cliente + maniobras_cliente  + iva_costo_cliente_cobro - ret_costo_cliente_cobro + iva_esta_cliente_cobro + iva_lavado_cliente_cobro + iva_flete_cliente_cobro + ret_flete_cliente_cobro + iva_demoras_cliente_cobro + iva_maniobras_cliente_cobro as total ',
        "id='$id'"
    );
    $result = $GLOBALS['a']->sql;
    $datos = $result->fetch_assoc();
